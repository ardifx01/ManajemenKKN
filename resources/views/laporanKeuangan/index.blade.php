<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <i class="fi fi-rs-chart-mixed-up-circle-dollar text-2xl leading-none relative"></i>
    ...
    <div class="my-4 bg-white border border-gray-200 rounded-lg shadow-sm">
        ...
        <div class="p-4 border-b border-gray-200">
            <div class="flex items-end justify-between gap-4 overflow-x-auto overflow-y-hidden whitespace-nowrap">
                ...
                <form action="{{ route('laporan-keuangan.index') }}" method="GET" class="flex items-center gap-2">
                    <div class="flex items-end gap-2">
                        ...
                        {{-- Jenis Transaksi --}}
                        <div>
                            <x-label for="jenis">Jenis Transaksi</x-label>
                            <x-select-input name="jenis" id="jenis">
                                <option value="" selected hidden>-- Pilih Jenis Transaksi --</option>
                ...
                <div class="flex items-end gap-2">
                    <a href="{{ route('laporan-keuangan.xls', ['startDate' => request()->startDate, 'endDate' => request()->endDate, 'jenis' => request()->jenis]) }}"
                    ...
                    <a href="{{ route('laporan-keuangan.pdf', ['startDate' => request()->startDate, 'endDate' => request()->endDate, 'jenis' => request()->jenis]) }}"
                ...
                <table class="min-w-full text-left text-gray-500">
                    <thead class="bg-gray-100 text-sm text-gray-700 uppercase">
                        <tr>
                            ...
                            <th scope="col" class="p-3 text-center">Metode Pembayaran</th>
                    ...
                    <tbody>
                            ...
                            <tr class="border-b">
                                ...
                                <td class="p-3">
                                    @if ($data->jenis == 'pemasukan')
                                        <span
                                            class="px-2.5 py-0.5 text-xs rounded-full bg-green-100 text-green-800">{{ ucfirst($data->jenis) }}</span>
                                    @elseif ($data->jenis == 'pengeluaran')
                                        <span
                                            class="px-2.5 py-0.5 text-xs rounded-full bg-red-100 text-red-800">{{ ucfirst($data->jenis) }}</span>
                                    @endif
                                </td>
                        ...
                        @endforelse
                        @if (count($keuangan) > 0)
                            <tr class="bg-gray-100 font-semibold text-gray-900 border-t">
                                <td colspan="4" class="text-end px-2 py-3">Pengeluaran:</td>
                                <td class="p-3 text-red-400 text-end">Rp.
                                    {{ number_format($pengeluaran, 0, ',', '.') }}</td>
                                <td colspan="2"></td>
                            </tr>
                            <tr class="bg-gray-100 font-semibold text-gray-900 border-t">
                                <td colspan="4" class="text-end px-2 py-3">Pemasukan:</td>
                                <td class="p-3 text-green-400 text-end">Rp.
                                    {{ number_format($pemasukan, 0, ',', '.') }}</td>
                                <td colspan="2"></td>
                            </tr>
                            <tr class="bg-gray-100 font-bold text-gray-900 border-t">
                                <td colspan="4" class="text-end px-2 py-3">Total Saldo:</td>
                                <td class="p-3 text-yellow-400 text-end">Rp. {{ number_format($saldo, 0, ',', '.') }}
                                </td>
                                <td colspan="2"></td>
                            </tr>
                        @endif
